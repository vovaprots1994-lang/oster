                    ...
                    // Else, we resize the image and return the new resized image url.
                        ...
                        if ( ! is_wp_error( $resized_file ) ) {
                            ...
                            if(!empty($attachment_id[0])){
                                $attachment_meta = get_post_meta($attachment_id[0],'_wp_attachment_metadata',true);
                                if(!empty($attachment_meta['sizes'])){
                                    $sizes = current( $attachment_meta['sizes'] );
                                    $sizes = $sizes['mime-type'];

                                    $attachment_meta['sizes'][] = array(
                                        'file' => str_replace($upload_info['url'].'/','',$img_url),
                                        'width' => $dst_w,
                                        'height' => $dst_h,
                                        'mime-type' => $sizes
                                    );
                                    update_post_meta($attachment_id[0],'_wp_attachment_metadata',$attachment_meta);
                                }
                            }
                        } else {
